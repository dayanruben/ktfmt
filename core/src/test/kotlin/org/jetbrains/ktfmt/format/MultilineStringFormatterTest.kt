...
class MultilineStringFormatterTest {
  ...
  @Test
  fun `format returns input unchanged when code cannot contain a trimmed multiline string`() {
    val code = "fun f() {\n  println(1)\n}\n"

    assertEquals(
        code, MultilineStringFormatter(4).format(Parser.parse(KotlinCode(code, FileType.REGULAR))))
  }

  @Test
  fun `getMultilineTrimmedStringList returns empty when code cannot contain a trimmed multiline string`() {
    val formatter = MultilineStringFormatter(4)

    assertTrue(
        formatter
            .getMultilineTrimmedStringList(
                Parser.parse(KotlinCode("fun f() { foo.trimIndent() }", FileType.REGULAR)))
            .isEmpty())
    assertTrue(
        formatter
            .getMultilineTrimmedStringList(
                Parser.parse(KotlinCode("val text = $TQ\nhello\n$TQ", FileType.REGULAR)))
            .isEmpty())
  }
