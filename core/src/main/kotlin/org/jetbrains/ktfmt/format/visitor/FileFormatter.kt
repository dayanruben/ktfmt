...
internal class FileFormatterImpl : FileFormatter {
  ...
  private fun formatFile(file: KtElement) {
    ...
    for (child in file.children) {
      ...
      val wasInScriptInitializerBeforeTrailingComment =
          state.inScriptInitializerBeforeTrailingComment
      state.inScriptInitializerBeforeTrailingComment =
          child is KtScriptInitializer && isBeforeTrailingComment(child)
      format(child)
      state.inScriptInitializerBeforeTrailingComment = wasInScriptInitializerBeforeTrailingComment
      builder.guessToken(";")
      builder.markForPartialFormat()
      prev = child
    }
