...
object Formatter {
  ...
  fun format(
    ...
    val formattedCode =
        if (characterRanges == null) {
          FormatterContext(splitLongStringLiterals(code, options))
  ...
  /**
   * Splits safe, long ordinary string literals before the normal pretty-printer runs, so the
   * resulting `+`-joined parts can be laid out within the width limit.
   */
  private fun splitLongStringLiterals(code: KotlinCode, options: FormattingOptions): KotlinCode =
      LongStringLiteralFormatter(options.maxWidth, options.continuationIndent).format(code)
