...
internal open class CallFormatterImpl : CallFormatter {
  ...
  override fun formatLambdaOrScopingFunction(expr: PsiElement?, emitLeadingBreak: Boolean) {
    ...
    if (tryFormatOneLineFunctionalInterface(scopingLambda)) {
      return
    }
  ...
  /**
   * Emits a short functional-interface constructor lambda on one line when it fits (issue 253).
   * For example `VeryLongInterface { EndResult }` stays on one line after an assignment break.
   */
  context(_: FormatterStateHolder)
  private fun tryFormatOneLineFunctionalInterface(scopingLambda: ScopingLambda): Boolean {
    val calleeText = scopingLambda.calleeExpression?.text ?: return false
    if (calleeText.firstOrNull()?.isUpperCase() != true) return false
    val lambdaExpression = scopingLambda.lambdaExpression
    val bodyExpression = lambdaExpression.bodyExpression ?: fail()
    val expressionStatements = bodyExpression.children
    if (
        expressionStatements.size != 1 ||
            ...
            !functionalInterfaceConstructorFitsOneLine(calleeText, lambdaExpression)
    ) {
      return false
    }
    ...
    if (valueParams.isNotEmpty() || lambdaExpression.functionLiteral.arrow != null) {
      builder.space()
  ...
  private fun functionalInterfaceConstructorFitsOneLine(
    ...
    return options.continuationIndent + callHead.trim().length + 1 + lambdaText.length <=
        options.maxWidth
  }
