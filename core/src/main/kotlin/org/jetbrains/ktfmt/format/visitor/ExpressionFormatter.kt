...
internal open class ExpressionFormatterImpl : ExpressionFormatter {
  ...
  override fun formatInitializerExpression(initializer: KtExpression, assignmentOp: String) {
    ...
    if (initializer.isLambdaOrScopingFunction) {
      ...
      formatBlockLikeExpressionWithHead(initializer)
    } else {
  ...
  /**
   * Whether [expression] is a `when` or a `with(...)` call whose head fits on one line, so it
   * can stay on the `=` line (issue 590).
   */
  context(_: FormatterStateHolder)
  ...
  context(_: FormatterStateHolder)
  private fun formatBlockLikeExpressionWithHead(expression: KtExpression) {
    ...
    builder.breakOp(
        Doc.FillMode.INDEPENDENT, " ", expressionBreakIndent, Optional.of(breakToExpr))
    ...
    if (expression is KtCallExpression) {
      ...
      formatFunctionCall(
          ...
          trailingLambda = null,
          argumentsIndent = expressionBreakIndent,
          lambdaIndent = ZERO,
      )
      val lambdaArgument = expression.lambdaArguments.singleOrNull() ?: fail()
      val lambdaExpression = lambdaArgument.getArgumentExpression() as? KtLambdaExpression ?: fail()
      builder.space()
      formatLambdaExpression(lambdaExpression, breakToExpr)
    } else {
      fail()
    }
  }
  ...
  override fun formatBinaryExpression(expression: KtBinaryExpression) {
    ...
    if (
        KtTokens.ALL_ASSIGNMENTS.contains(op) &&
            (expression.right.isLambdaOrScopingFunction ||
                isBlockLikeExpressionWithHead(expression.right))
    ) {
