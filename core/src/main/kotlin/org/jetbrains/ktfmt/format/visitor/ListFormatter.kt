...
interface ListFormatter {
  ...
  context(_: FormatterStateHolder)
  fun formatModifierList(list: KtModifierList, forceAnnotationLineBreaks: Boolean = false)
...
internal open class ListFormatterImpl : ListFormatter {
  ...
  context(_: FormatterStateHolder)
  override fun formatModifierList(list: KtModifierList, forceAnnotationLineBreaks: Boolean) {
    if (forceAnnotationLineBreaks && hasAnnotationWithArguments(list)) {
      formatFieldModifierListWithVerticalAnnotations(list)
      return
    }
    builder.sync(list)
  ...
  /** Emits each field annotation on its own line (issue 316). */
  context(_: FormatterStateHolder)
  private fun formatFieldModifierListWithVerticalAnnotations(list: KtModifierList) {
    builder.sync(list)
    var onlyAnnotationsSoFar = true
