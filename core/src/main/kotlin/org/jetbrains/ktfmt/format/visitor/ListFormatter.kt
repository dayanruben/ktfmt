...
interface ListFormatter {
  ...
  /**
   * Emits a single type-argument list as one unit without internal breaks, so that outer chain
   * breaks are preferred over breaking inside the chevrons (issue 207).
   */
  context(_: FormatterStateHolder)
  fun formatPreservedTypeArgumentList(list: KtTypeArgumentList)
...
internal open class ListFormatterImpl : ListFormatter {
  ...
  context(_: FormatterStateHolder)
  override fun formatPreservedTypeArgumentList(list: KtTypeArgumentList) {
    builder.sync(list)
    builder.token("<")
    state.preserveSingleTypeArgumentListsDepth++
    try {
      list.arguments.forEachIndexed { index, argument ->
        if (index > 0) {
          builder.token(",")
          builder.space()
        }
